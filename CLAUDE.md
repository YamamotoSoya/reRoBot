...
ros2 launch epos4_controller rerobot_bringup.launch.py
```
...
ros2 run epos4_controller epos4_odometry    --ros-args --params-file src/epos4_controller/config/params.yaml
```
...
- **`src/epos4_controller`** — application layer. Two executables share `config/params.yaml` (`tread_width`, `tire_diam`, `gear_ratio`, `invert_left/right`):
  ...
  - `launch/rerobot_bringup.launch.py` — composite bringup (bus_config + 5 s TimerAction + controller + odometry + robot_state_publisher).
...
- `tread_width`, `tire_diam`, `gear_ratio`, and `invert_left/right` are duplicated as ROS parameters in each consumer, sourced from `src/epos4_controller/config/params.yaml` (and a parallel copy in `src/epos4_teleop/config/params.yaml`). Keep them in sync if you change the chassis.
