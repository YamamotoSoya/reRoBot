...
- **`ros2_ws_main/src/bringup/rerobot_bringup`** — system bringup assets (no C++ code). Owns:
  ...
  - `urdf/rerobot.urdf` — 統合 robot description (2026-08-10 に rerobot_2d/3d.urdf を 1 本化)。`laser` / `rfans` / `imu_link` を常に含む。センサ位置は実測反映済み: laser (0.07, 0, 0.215)・rfans は取付プリセット依存 (下記)・imu (0, 0, 0.64)。IMU の向きは **rpy=(0, 0, π/2) (正立+90°) で最終確定** (2026-08-11, 実走 bag の 3 証拠: 静止 accel z=+9.815 / gyro_z⇔車輪 yaw rate 同符号 +1.015 / 裏返し URDF だと EKF yaw が鏡像)。⚠️ imu_joint を変えたら `ros2_ws_glim/config/config_sensors.json` の `T_lidar_imu` も要再計算。
    **rfans 取付プリセット** (2026-08-15 導入、丸ごとコピー方式): `rerobot.{tilted45,tilted15,flat}.urdf` + `ros2_ws_glim/config/config_sensors.{tilted45,tilted15,flat}.json`。tilted45 = 前傾 45° + ヨー 180° (現行)、tilted15 = 前傾 15°・ヨーなし (ケーブル前向き)、flat = 正立 (〜08-15 の標準)。切替は **URDF と GLIM の cp 2 発を必ずセットで** (コマンドは URDF 冒頭コメント参照。再ビルド不要 — URDF は symlink install、GLIM config は直接 mount)。⚠️ rfans 以外を編集したら同じ変更を全プリセットにも反映すること。
