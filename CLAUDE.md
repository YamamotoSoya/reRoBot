...
- **`src/rerobot_bringup`** — system bringup assets (no C++ code). Owns:
  ...
  - `launch/rerobot_bringup_3d.launch.py` — **3D LiDAR** bringup wrapper: common + `rfans_driver` (R-Fans, `src/external/StarROS2`, `respawn=True`/2 s)。frame_id `rfans`、出力 PointCloud2 `/sdk_could`。`device_ip` / `rps` / `model` を launch 引数で上書き可。`/scan` は出さない (LaserScan 変換は含まない)。
