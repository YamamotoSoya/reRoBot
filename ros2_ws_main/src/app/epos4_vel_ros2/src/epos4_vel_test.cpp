...
public:
  Epos4_Vel_Node() : Node("epos4_cmd_vel_node")
  {
    ...
    sine_wave_timer_ =
      this->create_wall_timer(10ms, std::bind(&Epos4_Vel_Node::timer_callback, this));
...
private:
    ...
    rclcpp::Client<std_srvs::srv::Trigger>::SharedPtr client, const std::string & service_name)
  {
    ...
    client->async_send_request(
      request, std::bind(&Epos4_Vel_Node::trigger_callback, this, std::placeholders::_1));
  }
  ...
  void call_target_vel_service(double target_vel_value)
  {
    ...
    if (target_vel_value != 0.0 && std::fabs(target_vel_value) < 1.0) {
      RCLCPP_ERROR(
        get_logger(),
        "Target velocity is too small. Enter an "
        "integer rpm value, for example 100 or -100.");
      return;
    }
    ...
    if (target_vel_rpm != target_vel_value) {
      RCLCPP_WARN(
        get_logger(), "Target velocity rounded from %f to %.0f rpm", target_vel_value,
        target_vel_rpm);
    }
    //-----
    ...
    client_target_->async_send_request(
      request, std::bind(&Epos4_Vel_Node::target_callback, this, std::placeholders::_1));
  }
