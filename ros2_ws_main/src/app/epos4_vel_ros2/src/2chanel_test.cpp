  ...
  void input_loop()
  {
    ...
    while (key != 'q') {
      ...
      if (key == 'w') {
        ...
        m1_value_ -= 10;
        m2_value_ -= 10;
        if (m1_value_ < -1 * max_speed) m1_value_ = -1 * max_speed;
        if (m2_value_ < -1 * max_speed) m2_value_ = -1 * max_speed;
      }

      if (js_arrived_m1_ && js_arrived_m2_) {
        RCLCPP_INFO(get_logger(), "Input Value = m1:%f m2:%f", m1_value_, m2_value_);
        RCLCPP_INFO(
          get_logger(), "Position Actual Value = m1:%f m2:%f", m1_joint_state_.position[0],
          m2_joint_state_.position[0]);
