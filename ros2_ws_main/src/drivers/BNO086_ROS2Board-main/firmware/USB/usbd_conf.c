/**
 * usbd_conf.c - glue between the ST USB device library and the HAL PCD driver.
 */
#include "usbd_core.h"
#include "usbd_cdc.h"
#include "board.h"
...
/* Static pool for USBD_malloc: one CDC class instance. */
static uint32_t s_class_pool[(sizeof(USBD_CDC_HandleTypeDef) / 4U) + 1U];
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  /* Packet memory layout, 1 KiB total on the F042.
   *   0x018  EP0 OUT   (64)
   *   0x058  EP0 IN    (64)
   *   0x0C0  EP1 IN    (64)  bulk data in
   *   0x100  EP2 IN     (8)  CDC notification
   *   0x110  EP1 OUT   (64)  bulk data out
   */
