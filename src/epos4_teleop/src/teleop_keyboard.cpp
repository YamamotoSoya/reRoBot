...
public:
  TeleopKeyboardNode() : Node("teleop_keyboard_node")
  {
    ...
    auto m1_topic =
      declare_parameter("m1_joint_topic", std::string("/motor1/cia402_device_1/joint_states"));
    auto m2_topic =
      declare_parameter("m2_joint_topic", std::string("/motor2/cia402_device_2/joint_states"));
...
private:
  ...
  void handleKey()
  {
    ...
    switch (ch) {
      case 'w':
        linear_ = std::min(max_linear_, linear_ + linear_step_);
        break;
      case 's':
        linear_ = std::max(-max_linear_, linear_ - linear_step_);
        break;
      case 'a':
        angular_ = std::min(max_angular_, angular_ + angular_step_);
        break;
      case 'd':
        angular_ = std::max(-max_angular_, angular_ - angular_step_);
        break;
      case ' ':
      case 'x':
        linear_ = 0.0;
  ...
  void render()
  {
    ...
    std::printf(
      ...
      free_mode_ ? "FREE/脱力" : "  DRIVE  ", linear_, angular_, linear_step_, angular_step_, l_m,
      r_m);
    std::fflush(stdout);
  }
