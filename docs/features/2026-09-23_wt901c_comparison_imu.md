...
**ステータス: 手順書 (2026-09-23)。準備ファイル作成済み。§2 のセンサ設定は 2026-09-23 に Windows 公式ソフト(新 UI、自動保存) で 115200 / 200 Hz / 帯域 98 Hz に変更し、電源断後も生フレーム計測で 4 種 ≈201 Hz・加速度フレームの連続同値 3% を確認 (保存済み・帯域有効)。§3 (submodule 追加・Qt5 apt・ビルド) と §4 (単体起動 200.5 Hz) も同日完了。**§5〜7 (同時起動・bag・比較) は未実施。**
