...
| シャシ | タイヤ径 **0.25 m** (08-30 車体改修、旧 0.15)。tread は**変更あり・実測待ち** (params は旧値 0.41 のまま TODO) | ⚠️ センサ搭載位置も改修で変更 — 実測・URDF 反映は未 (§10) |
| 減速比 | 物理 **92.25:1** = `gear_ratio: 92.25` (08-30 減速機交換、旧 5.0) | モータ/エンコーダは同一個体 → bus.yml `scale_pos_from_dev = 2π/1024` 据え置き。タイヤ 1 回転 = 94,464 inc。⚠️ 最高速はモータ 7720 rpm 制約で車体 **~1.09 m/s**。⚠️ 減速機交換で出力軸回転方向が反転している可能性 → invert_left/right の浮かせ確認が未 |
| 2D LiDAR | HOKUYO UTM-30LX (USB, urg_node) | frame `laser`, base_link から z+0.714 |
| 3D LiDAR | Sure-Star R-Fans-16 = **V6K-16G GM 格式** (Ethernet 192.168.0.3, `surestar_rfans_ros2` — 08-14 刷新) | frame `rfans`, PointCloud2 `/rfans_driver/rfans_points` (旧 `/sdk_could` は廃止)。**08-15 に前傾 45° + ヨー 180° 取付へ変更** (URDF rpy=(0,-π/4,π)、xyz 実測 (-0.02, 0, 0.75) 反映済み) |
...
0. (a) **トレッド幅の実測** → params.yaml ×2 の `tread_width` 更新 (旋回オドメトリに直結) →
   (b) **センサ位置の実測** → `rerobot.urdf` **全プリセット** (tilted45/tilted15/flat — rfans 以外の変更は全プリセット反映の規約) +
   imu 位置変更時は GLIM `config_sensors.json` の `T_lidar_imu` 再計算 (これも全プリセット) →
   (c) **浮かせ確認**: teleop 前進 → 両輪前転か (減速機交換で回転方向反転の可能性 → invert_left/right)、
   車輪 1 回転手回し → teleop 距離表示 ≈ 0.79 m (π×0.25) で較正確認 →
