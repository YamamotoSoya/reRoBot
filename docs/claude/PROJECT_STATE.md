...
5. **R-Fans ドライバ刷新** (08-14 開始、タイムライン 08-14 (2)): (a) ~~実機 UDP の device ID バイト確認~~
   ...
   **⚠️ 08-16 に最優先へ格上げ: 壁二重化 (z ドリフト→ループ不発) と取付角実験の崩壊の両方の根で、
   これを済ませるまで傾け取付の屋外評価は原理的に無意味**) → (g) メーカー問い合わせ (任意、連絡先は issue doc §7)
