...
5号館 08-14 手動 LC 地図 (344 m 周回、z ドリフト最大 +4.9 m — ループ閉じ後の値。閉じる前の
odom 軌跡では +2.85 m で、LC が端点を合わせる代わりに中間を持ち上げている) で、同一点群・
...
地図では段差が帯幅に対して効いてくる。また「未探索領域 = 自由」の制約 (§3.6 参照) は
...
## 3.5 glim_traj_to_2dmap — 生スキャン × 最適化軌跡の再投影 (2026-09-14 追加)

§3.4 の結論は「濃さを決めているのは GLIM の間引き粒度と submap の置かれ方」だった。
ならば点の出どころを **GLIM が間引く前 = bag の生スキャン**に戻せばよい。問題は「それでは
ループクロージングの恩恵を捨てることになるのでは」という点だが、答えは否で、しかも
GLIM を再実行する必要もない。

### ループクロージングは「点」ではなく「姿勢」を直す

```
GLIM の地図の中身と LC の作用点
├── submap i = 姿勢 T_i (世界のどこに・どの向きで置くか) + T_i から見た点の相対座標
│     点は submap 原点フレームで保存され、世界座標は表示・出力のたびに T_i × 相対座標で計算
├── ループクロージング = 「始端と終端は同じ場所」という拘束を足して T_0..T_222 を再最適化
│     点は 1 個も書き換えない。姿勢が動くので世界座標が変わるだけ
│     (5号館: 終端 submap の z が −4.37 → −0.07 に引き上がり、中間が z ドームになった)
└── 各スキャンの最終姿勢 = (所属 submap の最適化後姿勢) × (submap 原点から見たスキャンの相対姿勢)
      ↑ LC で動いた部分                                    ↑ オドメトリのまま (submap 内は剛体)
```

GLIM はこの掛け算を dump 保存時に全スキャンに対して行い、結果を dump 根元の
**`traj_lidar.txt`** に書く (`global_mapping.cpp` の save: `T_world_endpoint_L ×
T_odom_imu0⁻¹ × frame->T_world_imu`、TUM 形式 `stamp x y z qx qy qz qw`、1 行 = 1 スキャン)。
隣の `odom_lidar.txt` は掛け算前 = オドメトリ姿勢である。

| ファイル (5号館 LC dump) | 終端スキャンの z | 意味 |
|---|---|---|
| `odom_lidar.txt` | −4.50 m | ループクロージング前。始点から 4.5 m 沈んだまま |
| `traj_lidar.txt` | −0.02 m | ループクロージング後。閉じている |

だから「bag の生スキャンを `traj_lidar.txt` の姿勢で世界座標に置く」だけで、GLIM が自分の
(間引き済み) 地図を世界座標に置くのと同じ幾何が、間引き前の密度で得られる。submap 内の
相対姿勢がオドメトリのままなのは GLIM 自身の地図も同じなので、精度は落ちない。

⚠️ 条件が 2 つ: (1) offline_viewer で手動ループ閉じをしたら**保存し直した dump** を使う
(traj は保存時点の最適化結果)。(2) bag の header stamp と traj の stamp が一致すること —
GLIM はフレーム時刻に header stamp をそのまま使うので、5号館 bag では差 0.00 ms で
6,957/6,989 スキャンが一致した (不一致 32 = GLIM 起動前の 3 秒分)。

### 実装 — tools/glim_traj_to_2dmap

`tools/glim_traj_to_2dmap/glim_traj_to_2dmap.py` (numpy + rosbag2_py、glim_env で実行。
使い方の全文は `tools/README.md`)。処理はスキャンごとに次の 4 手:

```
1 スキャンの処理
├── header stamp で traj の姿勢 (R, t) を引く (最近傍、許容 --stamp_tol 0.02 s)
├── 高さ帯で点を選ぶ — 基準を 3 通りから選べる (--height_frame)
│   ├── sensor: センサ座標の z そのまま。実機 /scan (pointcloud_to_laserscan, base_link 相対)
│   │           と同じ切り方。姿勢の回転誤差を帯に持ち込まないが、車体の実ピッチや地面の
│   │           起伏で遠方の地面が帯に入る
│   ├── world : 世界 z − 姿勢 z。glim_dump_to_2dmap の sensor モードと同じ意味。
│   │           姿勢のピッチ誤差 (z ドーム斜面) がそのまま帯の傾きになる
│   └── ground: スキャンごとに近傍 (--ground_range 12 m) の低い点へ地面平面 z=ax+by+c を
│               最小二乗で当て (外れ値除去 2 回)、平面からの高さで切る。min/max_height は
│               地上高。取付の傾きにも姿勢誤差にも地面の起伏にも依存しない
├── 世界座標へ w = R·p + t (--deskew なら点ごとの time で前後姿勢を slerp 補間)
└── 画素へ投票 → 点数 → 既製互換の濃度変換 (min/max_points_in_pix)
```

速度は全 6,989 スキャン (5.8 GB bag) で約 25〜35 秒。最初の実装は `np.bincount(minlength=W·H)`
をスキャンごとに呼んで 19 分かかった — 6144² の配列を毎回確保していたためで、出現画素だけ
`np.unique` で加算する形に直して 40 倍速くなった (数値計算の落とし穴として記録)。

### 比較結果 (2026-09-14、5号館 08-14 LC 地図、同一 bag・同一 dump)

3 方式を同じ地図で並べた。数値は占有面積 (輝度 < 128 の画素 × 画素面積)、「遠/始」は右上 ROI と
始点 ROI の占有面積比 (1 = 濃さが場所に依らない)、「一致率」は 278 スキャンをサンプルして帯内点を
最適化姿勢で置いたとき地図の占有セル (±1 px) に乗る割合。⚠️ 一致率は J 以降の方式では
**同じスキャンが地図に入っている自己包含の値**で、critic 査読の hold-out 再計算 (偶数 40 s
ブロックで地図を作り奇数ブロックで試験) では J 0.79・Q 0.92・B 0.77 と差が縮む。詳細は考察。

![3 方式の比較 — 左: 全体、右: 右上ズーム](img/2026-09-14_traj_compare_grid.png)

| 変種 | 点の出どころ | 高さ帯の基準 | r / 点数 | 占有 [m²] | 右上 ROI | 始点 ROI | 遠/始 | 一致率 (自己包含) |
|---|---|---|---|---|---|---|---|---|
| I | submap (間引き済) | 絶対 z +0.13〜+1.33 | 0.05 / 2..5 | 109 | 0.0 | 21.9 | 0.00 | 0.23 |
| A | submap | 姿勢 z 相対 −0.25〜+0.95 | 0.05 / 2..5 | 138 | 3.2 | 30.5 | 0.10 | 0.38 |
| B | submap | 同 | 0.10 / 2..5 | 816 | 54.5 | 145.4 | 0.37 | 0.77 |
| J | **生スキャン** | センサ z −0.35〜+0.85 | 0.05 / 2..5 | 2,514 | 462 | 380 | 1.22 | 0.97 (hold-out 0.79) |
| M | 生 | 同 + deskew | 0.05 / 2..5 | 2,443 | 448 | 368 | 1.22 | 0.96 |
| L | 生 | 世界 z − 姿勢 z −0.25〜+0.95 | 0.05 / 2..5 | 2,468 | 525 | 335 | 1.57 | 0.94 |
| P | 生 | センサ z、range 2.5〜30 m | 0.05 / 4..12 | 1,652 | 319 | 252 | 1.26 | 0.92 |
| Q | 生 | **地上高 +0.3〜+1.5 (平面当て)**、range ≤30 m | 0.05 / 4..12 | 1,638 | 315 | 250 | 1.26 | 0.91 (hold-out 0.92) |
| **R** | 生 | 同 + deskew | 0.05 / 4..12 | 1,604 | 310 | 244 | 1.27 | 0.91 |
| R10 | 生 | 同 | 0.10 / 8..24 | 2,232 | 414 | 351 | 1.18 | 0.93 |

![詳細 — 上: 右上のセンサ z 帯 (J) vs 地上高帯 (Q)、中: deskew 有無、下: 軌跡の左 3 m の黒帯](img/2026-09-14_traj_detail.png)

![一致率と画素点数分布](img/2026-09-14_traj_match.png)

### 考察 — 直ったこと・直っていないこと・分かったこと

```
生スキャン再投影で変わったもの
├── ✅ submap 数依存の薄化は解消 — 右上/始点の占有比 0.10 (A) → 1.2 (J〜R)。壁が線でつながり、
│     右上にも建物・低い植栽の列・柵が出る。密度は「速度・距離依存」に戻った (物理的に正しい不均一)
├── ✅ 帯の追従が submap 単位から 1 スキャン単位になった
├── ⚠️ 一致率の「最も一致する」は自己包含の見かけ — hold-out で J 0.79 ≒ B 0.77。姿勢を横に
│     0.5 m ずらしたときの落ち幅 (識別力) はどの方式も +0.3 で差がない。AMCL が実際にどちらで
│     収束しやすいかは bag 再生 + AMCL の実験でしか決まらない (未実施、下記「次の 1 実験」)
├── ⚠️ 幾何は「submap 姿勢と同じ LC 状態で剛体配置」— critic が dump の submap 姿勢から
│     フレーム単位で再計算し traj 行と 0.4 mm 一致を確認。ただし GLIM 自身の地図は deskew 済み
│     (global_shutter_lidar: false) なので、点配置を揃えるなら --deskew (M/R) を使う。
│     J と M の占有 IoU は 0.78 = 回転区間で 22% のセルが違う
├── 📌 軌跡の左 3 m に沿う黒帯 = 地上高 0.3〜0.6 m の**静止物**。根拠: range_min 2.5 でも残る /
│     その位置の点の地上高は 42% 地面・38% が 0.3〜1.5 (上側の山 0.47〜0.63 m、胴体高さの山なし) /
│     観測時刻幅 p10–p90 = 51 s (随伴物なら十数秒。6〜8 m 左の明白な静止物と同じ幅) /
│     別 run (08-14 0846) にも同区間で同じ線と同じ開口。実体 (植栽・縁石・低柵・自転車ラック等) は
│     未確認 — Nav2 上は何であっても障害物なので運用に影響しない
├── 📌 遠方の筋状ノイズ (J) の主因は **取付ピッチ ≈ 1.2° の系統誤差** — スキャンごとの地面平面
│     フィットでピッチ中央値 −1.29° [p5 −1.88, p95 −0.68]、ロール +0.06° (取付ロールなし)。
│     30 m 先で 0.63 m の高さ誤差になり、遠方地面がセンサ z 帯に入る。車体の揺れ・起伏は残差 ±0.5°
│     → 地上高帯 (Q/R) はこれを吸収するが、実機 /scan には同じ筋が残るので「地図と観測の切り方
│     を揃える」観点では J 側にも理がある (用途で選ぶ: AMCL 用は R、static costmap 用も R が安全側)
└── 📌 LC 後の traj は z 最大 +4.95 m (odom 最大 +2.85 m) — LC は端点を閉じただけでドームは残り、
      むしろ中間の z を持ち上げている。§3.4 の「+4.9 m」は LC 後の値
```

残っている代替仮説 (critic 査読で棄却できなかったもの):

- 左 3 m の帯が「3 m 離れて並走した随伴者」である可能性。観測時刻幅 51 s と別 run の一致で
  不利だが完全棄却には現地写真か、0846/0919 の 2 run を建物線で位置合わせして同一世界座標に
  出るかの確認が要る (帯の蛇行 sd 0.98 m が軌跡の蛇行と相関 0.63 な点は随伴物側に少し傾く)
- 一致率の差が「地図の黒面積の差」に過ぎない可能性。hold-out と null 基準 (0.5 m ずらし) で
  識別力が同等と出たので、方式選択の根拠を一致率に置くのはやめる

### 推奨と注意

```
推奨 (2026-09-14 時点)
├── 地図生成の既定: R = --height_frame ground --min_height 0.3 --max_height 1.5
│                      --range_max 30 --deskew --min_points_in_pix 4 --max_points_in_pix 12
│     地上高指定なので /scan 側 (pointcloud_to_laserscan の base_link 相対 min/max_height) と
│     同じ単位で帯を揃えられる。Nav2 用に 0.10 m 格子が要るなら R10 (点数 8..24)
├── 生スキャン系の共通の副作用
...
├── 濃さ = 重なった submap 数 (GLIM 0.3 m 間引き) → 生スキャン × traj_lidar.txt の再投影で
│     物理密度に戻す (glim_traj_to_2dmap, §3.5)。LC は姿勢を動かすだけなので traj だけで反映される
