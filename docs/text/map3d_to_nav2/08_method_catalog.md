...
│   │     └ I-2b 生スキャン × 最適化軌跡の再投影 .. glim_traj_to_2dmap ★reRoBot 自作済 (第3章 §3.5, 2026-09-14)
...
#### 2026-09-14 追補 — I-2b: 生スキャン × 最適化軌跡の再投影

I-2 の弱点「SLAM 地図の点は間引き済みで密度が submap の置かれ方に依存する」を、点の出どころを
bag の生スキャンに戻して解く変種。SLAM 側からは**姿勢だけ**を借りる (GLIM なら dump 根元の
`traj_lidar.txt` = ループクロージング反映済みの 1 スキャン 1 姿勢)。ループクロージングが submap
姿勢を剛体で動かすだけである以上、姿勢さえ最適化後なら生点群を置き直すだけで閉じた地図になる。
RTAB-Map の「ノード毎に局所格子 → 最適化姿勢で合成」と同じ思想を、オフラインで SLAM 非依存に
実装した形。高さ基準は 3 通り (センサ z / 世界 z − 姿勢 z / スキャン毎の地面平面) で、地面平面
基準は I-5 (地面分離ファースト) をスキャン単位で行う簡易版に相当する。

- 効果 (5号館 LC 地図): 右上/始点の占有比 0.10 → 1.2、壁が線でつながる。所要 30 s / 7,000 スキャン
- 弱点: 密度が速度・距離依存になる (停車区間が濃い)、動体が残る、未知領域は依然「自由」。
  「実機 /scan と最も一致する」は hold-out 評価では submap 版 r=0.10 と同等で、AMCL 実走での
  優劣は未検証 (第 3 章 §3.5 の考察)
...
- 弱点: 完成済み静止点群にはレイ情報が無いので効果半減 (第 3 章 §3.6 で却下した
...
│   ├── I-2b 生スキャン × traj 再投影 (glim_traj_to_2dmap) . 第3章 §3.5 — 濃さの submap 依存を解消 (09-14)。AMCL 実走比較が未
