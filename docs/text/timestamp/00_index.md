...
│   ├── bag による時刻整合の検証手順
│   └── 新センサ導入時の stamp 検収項目
└── 第8章 巻き戻し誤差の解剖 ........ 08_rfans_rewind_anatomy.md
    ├── IMU×LiDAR の対応付けは「最近傍」ではない (積分+補間)
    ├── ハード (回転ムラ) の無罪証明 — 時刻と角度は両方実測
    ├── 誤差の発生点 = アンカーの now() 1 行
    ├── ブラックボックス SDK の罪と無罪
    └── 誤差バジェットと対策の階段 (テンプレ3 化 / PPS 同期)
```
...
- **第8章は第5章 5.6 節の懸念 1 (受信時刻アンカー) の深掘り** (2026-08-13 追章)。
  「IMU と LiDAR の時刻対応はどれだけ信用できるのか」が気になったらここ。
  前提は第3章 (テンプレ2/3) だけなので、3 → 8 と飛んでも読める。
