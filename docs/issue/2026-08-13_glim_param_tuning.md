...
ほぼ平ら)。廊下の実勾配はユーザ確認で**無し** (2026-08-14「床はずっと水平」) →
生データ調査 (下記) で床下ゴーストが真犯人候補に浮上。

### 生データ調査 (2026-08-14) — 床下ゴースト = ワックス床の鏡面反射

「点群の段階で床より極端に低い点は無いか、遠いほど沈まないか」(ユーザ指摘) を
bag 冒頭の静止 2 s の生 `/sdk_could` (53.5 万点) で検証:

![生データの床異常](img/2026-08-13_glim_param_tuning/raw_floor_probe.png)

```
判明したこと (静止・生データ)
├── ⭕ 床下の点が大量にある: z<-1.0 m が全点の 9.4%、最深 -3.2 m (床は -0.725)
│   └── 左パネル: 下向きビームが床との交点で止まらず「レイの延長線上」に
│       床下へ点列が続く = ビームがワックス床で鏡面反射して壁などに再入射し、
│       往復距離ぶん遠い偽点を元のレイ方向に置く【鏡面反射ゴースト】
├── ⭕ ゴースト率は距離で激増: 近距離 10-24% → r=8-16 m で 3-10% → r>23 m は 40-60%
│   └── 遠方の「地面付近」はゴーストが本物を上回る
├── ⭕ 「遠いほど沈む」の逆で、床帯の中央値は遠いほど「浮く」(-0.70 → -0.45)
│   └── 斜入射では拡散反射 (本物の床の戻り) が消え、幅木・ドア下などの
│       床より高い低所オブジェクトが「床帯」に残るため
└── ⭕ ビーム別の床読みも ±0.3-0.4 m ばらつく (bank B の近距離ビームは
    ゴースト支配で中央値が床下 -1.13 m に沈む)
```

**z 沈みへの寄与メカニズム (仮説強化)**: GLIM が z のアンカーにできる「本物の床」は
r < 5 m 程度だけで、その外側の地面付近は「浮いた低所オブジェクト + 沈んだ鏡面
ゴースト」の混合。ロボットが前進すると視点によって混合比が変わり、地図と新スキャンの
「床」がずれて z が滑る。E9 (古い点が残る設定) で悪化したこととも整合する。

**対策候補**:
1. **E14 (config のみ・5 分)**: preprocess の `enable_cropbox_filter` で LiDAR 座標系の
   床下 (z < -0.95) を丸ごと除去 → 鏡面ゴーストを入口で殺す。ピッチ ±1° の実走でも
   r<20 m の本物の床は z > -0.95+α で残る
2. ドライバ側で intensity / pulseWidth によるゴースト除去 (鏡面ゴーストは戻りが弱い) —
   要調査
