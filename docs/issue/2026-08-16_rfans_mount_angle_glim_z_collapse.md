...
- ⚠️ **2026-09-10 追記**: 「縦角表が本命」は同種の因果実験 (縦角を補正して再実行 → z_end が run 間ばらつき内) で **主犯ではない**と判定 (≤30% の副次寄与は未確定)。「ヨー 180° で符号反転 = センサ固定誤差」の傍証も、45° 側が照合縮退で別原因と本文自身が認定しているため比較不成立。z ドリフトの正体は「世界固定傾き A + 一定沈降 c」で、15° 取付は A を増幅する条件 (地面観測の前後非対称化) と読み替える。A の発生源は未特定。なお本 doc の m16 実験 (extrinsic 1° 補正で不変) は tilted15・n=1 であり、flat 構成の extrinsic 無罪の根拠には使えない (critic 指摘)。対策 1 (縦角再較正が全ての前提) は取り下げ、対策 2・3 (45° 不採用・flat 運用) は有効。詳細 `2026-09-10_glim_z_drift_not_vangle.md` (§7)
