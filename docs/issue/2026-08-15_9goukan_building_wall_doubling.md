...
- ⚠️ **2026-09-10 追記**: 本文の連鎖「縦角誤差 → z ドリフト → ループ不発 → 二重壁」のうち **先頭の「縦角誤差」は主犯ではない** (縦角を補正して GLIM を再実行しても z_end が run 間ばらつき内。≤30% の副次寄与は未確定)。z ドリフトの正体は「単一の世界固定傾き A + 一定沈降 c」で、A の発生源は未特定 (時刻オフセット / LiDAR–IMU extrinsic 回転 / 初期重力整合が候補、屋外の z 観測性欠如は増幅条件)。「z ドリフト → ループ不発 → 二重壁」の後段は有効。対策候補 1 (縦角再較正) は優先度を下げ、候補 2〜4 が残る。詳細 `2026-09-10_glim_z_drift_not_vangle.md` (§7 critic 査読込み)
