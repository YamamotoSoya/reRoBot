...
**✅ 2026-07-29 完了** — 実測の結果 EPOS 側 (0x3010:01=256) は正しく、修正は **ROS 側 2 点同時** (bus.yml 2π/1024 + gear_ratio 5.0) に簡約されて適用・浮かせ検証済み。以下の旧手順は記録として残す (「3 点同時」は EPOS 誤設定仮説が前提で、実測により不要と判明)。**残タスクは下記 3. の接地検証のみ** — ⚠️ 修正により実車速度が従来の 4 倍 (=指令どおり) になっているので低速から。詳細は encoder_scaling_4x issue の「実測記録 2026-07-29」。

<details><summary>旧手順 (2026-07-29 以前の計画)</summary>
