...
### 15.6 追記 (2026-09-25、critic 査読) — §15.2 の旋回同期誤差の正体は BNO086 ドライバの auto_tare

`docs/issue/2026-09-24_bno086_auto_tare_rotates_motion_outputs.md` §2.7 参照。要点:
- §15.2 の「旋回同期パルス 6〜7°/90°」は生ジャイロの誤差ではなく、ドライバ既定 `auto_tare: all` が出力座標に掛けていた定数回転で、公開座標の gyro 回転軸が z から 4.8〜7.3° 傾いていたもの (**確定**: 0803/0918/0922 bag で CCW/CW 同符号・同大、tare-off bag `2026-09-25_0751` では 0.5° = WT901C と一致)。
- 回転の源は加速度計 x バイアス (+0.7〜1.2 m/s²) を fusion が基板傾きと誤認したもの (**本命**、床傾き 1〜2° の寄与は未分離)。
- **撤回**: 項 4 の「加速度計仲裁 → ジャイロ側の誤り」は融合姿勢が同じ accel から作られるため識別力なし。項 5 の「方向反転で固定座標ずれ棄却」は tare yaw が起動ごとに変わる事実を見落とし。E1 (ジャイロ置換 run) は未実施のまま。
- **残るもの**: gyro 積分 vs BNO086 融合姿勢の差 (§15.2 の指標) は tare off でも 3.5°/比 0.061 残る (accel バイアスが融合姿勢を傾ける)。過去 bag は R_tare の傾き・方位 (0803 7.0°/−8°、0918 7.3°/−9°、0922 4.8°/−144°) で gyro/accel を同時に逆回転すれば物理軸に戻せる。
- **BNO086 は物理的に上下逆 + yaw −93°** で、URDF と live の T_lidar_imu は 09-24 に更新済み。〜09-23 の bag を再処理するときは `config_sensors.flat_tareall_legacy.json` (旧 Rz 90°) を使う (新値は tare 座標に対して gyro_z 反転で破綻する)。
